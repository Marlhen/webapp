...
# Cargar datos
@st.cache_data(ttl=600) # Se añade ttl para refrescar datos cada 10 min
# Cargar datos
# MODIFICACIÓN: Reducimos el TTL a 60 segundos (o lo quitamos si quieres tiempo real estricto)
# Si quitas el ttl y el decorador, será más lento pero siempre actual.
# Con ttl=60, se actualiza automáticamente si ha pasado 1 minuto desde la última carga.
...
def cargar_datos(archivo_path):
    ...
    # ... (Resto de la lógica de limpieza de fechas igual que antes) ...
    ...
    # Renombrar columnas de fechas en el dataframe
    columnas_nuevas = encabezados[:10] + fecha_cols
    # Agregar columnas adicionales si hay más
    if len(encabezados) > len(columnas_nuevas):
        columnas_nuevas += [f'Extra_{i}' for i in range(len(columnas_nuevas), len(encabezados))]

    df_datos.columns = columnas_nuevas[:len(df_datos.columns)]

    # Convertir columnas de fechas a numérico y reemplazar NaN/NaT con 0
    for fecha_col in fecha_cols:
        if fecha_col in df_datos.columns:
            df_datos[fecha_col] = pd.to_numeric(df_datos[fecha_col], errors='coerce')
            df_datos[fecha_col] = df_datos[fecha_col].fillna(0)
...
def exportar_grafico_png(figura, ancho=1200, alto=600):
    ...
    try:
        ...
        st.warning(f"⚠️ Error exportando gráfico: {str(e)}")
        return None
...
def generar_pdf_reporte(df_filtrado, fecha_cols, fecha_cols_display, tipo_seleccionado, fig_linea, fig_barras, fig_pie, fig_heatmap, fig_acum):
    ...
    if fig_linea is not None:
        try:
            # ✅ Aplicar tema blanco para mejor contraste en PDF
